...
_HOUSE_PLAN_ENGINE_VERSION = "18"  # bumped: solver.py now scores/reports each room's Vastu zone from its rect centre instead of its top-left corner — a room could anchor its corner in a cheap pada while its actual bulk sprawled into a completely different (often avoid-listed) zone, e.g. kitchen_dining's corner pinned southeast while the room itself filled the southwest quadrant the master bedroom needed. Also: layout.py no longer hardcodes vastu_region "center" on every leftover open/circulation fragment (corridor scraps, carve leftovers) — only the one fragment that keeps the corridor spine's own center_id does; every other open fragment now reports its real compass zone, so the frontend's "largest brahmasthan-kind fragment wins the ब्रह्मस्थान label" no longer lets a big merged scrap off in some corner outrank the true (deliberately thin) centre band.
